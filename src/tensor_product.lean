...
{ default := ⟨universal_property.factor hf,
  ...
  unique  := λ ⟨h, H, hh⟩, subtype.eq $ funext $
    universal_property.factor_unique hf h H hh }

variables {β γ α₁}
protected theorem ext {f g : β ⊗ γ → α₁} (z : β ⊗ γ) (hf : is_linear_map f) (hg : is_linear_map g)
  (h : ∀ x y, f (x ⊗ₛ y) = g (x ⊗ₛ y)) : f z = g z :=
...
{ to_fun    := hba3,
  inv_fun   := hb1,
  left_inv  := λ x, tensor_product.ext x (hb2.comp hba4) is_linear_map.id hbb1,
...
have hbb3 : _ := λ x, tensor_product.ext x hbb2 is_linear_map.id hbb1,
have hgg1 : ∀ y x, (hbg3 ∘ hgb3) (y ⊗ₛ x) = y ⊗ₛ x,
from λ x y, by simp [function.comp, *],
have hgg2 : is_linear_map (hbg3 ∘ hgb3) := hbg4.comp hgb4,
have hgg3 : _ := λ x, tensor_product.ext x hgg2 is_linear_map.id hgg1,
{ to_fun    := hbg3,
  inv_fun   := hgb3,
  left_inv  := hbb3,
  right_inv := hgg3,
...
have hb6 : ∀ x, ha3 (hb3 x) = prod.inl x :=
  λ x, tensor_product.ext x (ha4.comp hb4) prod.is_linear_map_prod_inl $ λ x y, calc
          ha3 (hb3 (x ⊗ₛ y))
...
have hc6 : ∀ y, ha3 (hc3 y) = prod.inr y :=
  λ y, tensor_product.ext y (ha4.comp hc4) prod.is_linear_map_prod_inr $ λ x y, calc
          ha3 (hc3 (x ⊗ₛ y))
...
have h2 : _ := λ x, tensor_product.ext x h1 is_linear_map.id
  (λ x y, by simp [function.comp, *, add_tprod.symm]),
...
{ add_pair  := λ m n k, (ha4 k).add m n,
  pair_add  := λ m n k , tensor_product.ext m (ha4 $ n + k) ((ha4 n).map_add (ha4 k)) $ λ x y, calc
            ha3 (x ⊗ₛ y) (n + k)
  ...
  pair_smul := λ r x y, tensor_product.ext x (ha4 $ r • y) (ha4 y).map_smul_right $ λ m n, calc
            ha3 (m ⊗ₛ n) (r • y)
...
{ add_pair  := λ m n k, tensor_product.ext k (hb4 $ m + n) ((hb4 m).map_add (hb4 n)) $ λ x y, calc
            hb3 (m + n) (x ⊗ₛ y)
  ...
  smul_pair := λ r x y, tensor_product.ext y (hb4 $ r • x) (hb4 x).map_smul_right $ λ m n, calc
            hb3 (r • x) (m ⊗ₛ n)
...
have hc1 : _ := λ z t, tensor_product.ext t (hb8.comp $ ha6.linear_pair z) (tprod.is_bilinear_map.linear_pair z) $ λ x y, calc
        hb7 (ha3 (x ⊗ₛ y) z)
      = hb7 (x ⊗ₛ (y ⊗ₛ z)) : congr_arg hb7 (ha5 z x y)
  ... = hb3 x (y ⊗ₛ z) : hb9 x (y ⊗ₛ z)
  ... = (x ⊗ₛ y) ⊗ₛ z : hb5 x y z,
have hc2 : _ := λ t, tensor_product.ext t (hb8.comp ha8) is_linear_map.id $ λ xy z, calc
        hb7 (ha7 (xy ⊗ₛ z))
      = hb7 (ha3 xy z) : congr_arg hb7 (ha9 xy z)
  ... = xy ⊗ₛ z : hc1 z xy,
have hd1 : _ := λ x t, tensor_product.ext t (ha8.comp $ hb6.pair_linear x) (tprod.is_bilinear_map.pair_linear x) $ λ y z, calc
        ha7 (hb3 x (y ⊗ₛ z))
      = ha7 ((x ⊗ₛ y) ⊗ₛ z) : congr_arg ha7 (hb5 x y z)
  ... = ha3 (x ⊗ₛ y) z : ha9 (x ⊗ₛ y) z
  ... = x ⊗ₛ (y ⊗ₛ z) : ha5 z x y,
have hd2 : _ := λ t, tensor_product.ext t (ha8.comp hb8) is_linear_map.id $ λ x yz, calc
        ha7 (hb7 (x ⊗ₛ yz))
